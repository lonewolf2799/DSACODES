...
int lcs_DP(string s, string t) {
	...
	for(int i = 1; i <= m; i++) {
		for(int j = 1; j <= n; j++) {
			...
			if(s[m-i] == t[n-j]) {
				output[i][j] = 1 + output[i-1][j-1]; // match condition as when the character matches, we exclude both of the characters that matched and go to their substrings starting from the next character
			}
			else {
				int a = output[i-1][j];
				int b = output[i][j-1];
